function chart() {
    console.log('This is the chart page!');
    $('h2').text('Chart');
    $('#chart').show();
    //$('#resetBtn').show();
    $('#uploadBtn').show();
  ...
  function volcanoPlot() {
    ...
    //   //queue
    //   var q = d3.queue();
    //     q.defer(d3.csv, '/upload')
    //     .await(loadFile);
    
        //check file extension before giving error
        $("#uploadFile").change(function () {
            var fileExtension = ['csv'];
            if ($.inArray($(this).val().split('.').pop().toLowerCase(), fileExtension) == -1) {
                alert($.inArray($(this).val().split('.').pop().toLowerCase()));
                alert("Only formats are allowed : "+fileExtension.join(', '));
            }
        });

        var file, x, dirNameVal;
        //form submit
        $("#csvData").submit(function(e) {
            file = document.querySelector('input[type=file]').files[0];
                x = document.getElementById('noFileErrorMsg');
                dirNameVal = document.getElementById('dirName').value;
            ...
            var xhr = $.ajax({
              ...
              success: function(data){
                  ...
                  if(!file && !dirNameVal){
                    ...
                    $('#resetBtn').show();
                        d3.csv( data, parser, function(error,data){
                            d3.select('#chart')
                            .data([data])
                            .call(volcanoPlot);
                            console.log([data]);
            ...
            var xhr2 = $.ajax({
                url: "/api/returnFileDir",
                type: "POST",
                header : {
                    'Content-Type': undefined,
                    'path': $('input[name=dirName]').val()
                },
                data: $('input[name=dirName]').val(),
                dataType : "text",
                processData : false,
                ...
                success: function(data){
                    ...
                    if(data == "true")
                    {
                        alert('file uploaded to directory.');
                        
                    }else{
                        alert('folder does not exist. creating now.');
                    }
      ...
      function loadFile(){
          ...
          reader.addEventListener("load", parser, false);
          if(file)
          {
            $('#resetBtn').show();
            d3.selectAll("svg").remove();
            d3.csv('/upload', parser,function(error, data){
